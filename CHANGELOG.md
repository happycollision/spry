...
- `sp group --apply`'s input schema (`src/parse/apply-doc.ts`) now makes `sp view --json` output valid `--apply` input **verbatim**, so `sp view --json | sp group --apply -` round-trips with no field-stripping. Two changes: (1) the input PR directive field is renamed from `pr` to `prAction` (`"prAction": "CLOSE" | "ADOPT"`), distinct from the output `pr` state object so the two never collide; (2) the three output-only fields `view --json` emits on every node — `sha`, `subject`, and `pr` — are now silently **ignored** on input rather than rejected (previously any `sha` on input was a hard parse error). The internal parsed representation (`ParsedCommit`/`ParsedGroup`) was renamed to match: the field is now `prAction`, not `pr`.
- `sp group --apply`'s `reconcile` now requires `prAction:"CLOSE"` acknowledgment whenever an open PR would be **abandoned** by the apply, not only when its unit is reissued. A live id with an open PR is abandoned if, after the apply, no unit holds that id as its identity. Two new cases are caught: **(1) a member absorbed into a non-adopting group** — a commit with an open PR becomes a member of a group whose resolved id is a fresh mint or a _different_ member's id — now requires `prAction:"CLOSE"` on that member node (`plan.prCloses` gets the member's id); adopting the member's own PR (`prAction:"ADOPT"` with a matching group id) is unaffected and needs no close. **(2) dissolving a group whose own (minted, non-member) id still holds an open PR** — listing its members top-level with no node left to carry an acknowledgment — is now a hard error (`Group <id> holds an open PR but is being dissolved; dissolution cannot acknowledge a PR close. Reissue or restructure instead.`), since a final-state doc has no node to declare a close for an id that no longer exists in it. `reconcile` computes this via a whole-doc `heldIds` set (every id a unit ends up holding: unreissued top-level commit ids plus every group's resolved id) built alongside the existing per-node pass, then a post-pass diffs `live.openPrIds` against it — but **exempts ids already acknowledged** (`plan.prCloses`) from that hard error, so an _adopted_ group id (as opposed to a minted, non-member one) whose close was already carried by a surviving node — dissolved-and-reissued, or absorbed into another group with `prAction:"CLOSE"` — reconciles instead of double-erroring. Also swept the six leftover `pr:` error-message strings from the task-14 rename (`prAction:"CLOSE"`/`"ADOPT"` throughout).
- `sp sync` now writes structured, reusable PR bodies. New PRs get spry-owned marker regions — an info line, a `spry:body` section (single-commit prose or a grouped-commit subject list), a `spry:stack-links` section (the stack rendered newest-first with manual descending numbering), and a beta footer — and the user region below the body is seeded with the repo's PR template when one exists (`.github/PULL_REQUEST_TEMPLATE.md` etc.). On every subsequent `sp sync`, the spry-owned regions are re-derived and spliced back in place, rewriting the PR body only when it actually changed; anything you write outside the markers is preserved byte-for-byte. Missing regions self-heal (a pre-feature or hand-edited body gains the marker regions on the next sync). Stack-links converge in a single `sp sync --open` run (just-created PRs are linked into their siblings immediately). New helpers: `buildInitialBody`/`spliceBody`/`generateStackLinks`/`generateBodyContent` (`src/gh/pr-body.ts`), `findPRTemplate` (`src/gh/pr-template.ts`), `fetchPRBody`/`updatePRBody` (`src/gh/pr.ts`); the end-of-sync `updateStackBodies` pass in `src/commands/sync.ts`. **Cassettes re-recorded** because create-time PR bodies (sent on `gh pr create` stdin, a replay match key) changed and sync now makes `gh pr view`/`gh pr edit --body-file` calls per open PR. Note: a PR originally opened by a pre-marker spry version has its existing body treated as user text, so its first post-upgrade sync appends the spry regions below it (the commit body may appear once more); subsequent syncs are stable.

- `sp view` now marks units that have drifted since your last sync: `✎` (yellow) when you have local edits not yet pushed — the unit's tip differs from the SHA `sp sync` last pushed for it (run `sp sync`) — and `↓` (cyan) when the remote-tracking ref moved since your push, as of your last fetch. The two signals are independent and compose (`✎↓` = both). Both surface in `--json` as `localAhead` / `remoteAhead` booleans on each node. Fully offline: `sp view` never fetches, and a signal is shown only when its reference point is known (an old PR-cache entry with no recorded synced SHA, or a missing remote-tracking ref, shows no marker rather than a false positive), so fully-clean stacks render exactly as before. `sp sync` records the pushed tip as `syncedHeadSha` in the PR cache (`src/gh/pr-cache.ts`); `sp view` classifies drift via a new pure `classifyDrift` (`src/git/drift.ts`) against that SHA and the remote-tracking tip resolved by `resolveRemoteTrackingTip` (`src/git/branch.ts`). (spry-ywa8)

### Fixed

- `sp sync --open` now caches the PR it just opened on the same run, so `sp view` shows the new PR's status (and the `sp view` drift baseline `syncedHeadSha`) immediately instead of after a second `sp sync`. Previously the PR-status lookup was taken once at the start of the run — before any PR was created — and the post-creation cache write only ran when a retarget pass happened, so a freshly-opened PR (e.g. a single-unit stack or a new group) was left out of the local cache (`refs/spry/prs`) entirely: `sp view` rendered it as `○ no PR`, and — because the drift feature keys off the cached `syncedHeadSha` — a local edit after `--open` could show only `↓` (remote-tracking, which lives in git refs) and never `✎`. Fix: the just-created PRs are folded into the map used for the rest of the run, and the cache is written from that map whenever `--open` was requested (not only on retarget). To avoid writing the cache ref twice (a redundant push and a duplicate `✓ Updated PR cache` line), `checkSync` now takes a `skipCacheWrite` flag that `sp sync` sets on `--open` runs, deferring the single write to the post-open pass that has the complete picture. Covered by a new regression test in `tests/commands/sync.test.ts` (a fresh `--open` leaves an `OPEN` cache entry with `syncedHeadSha` set to the pushed tip). (spry-5364)
- Interactive TUIs (`sp group`, and the `sp sync --open` / `sp land` pickers) no longer scroll the terminal a full page on every keypress, leaving a trail of stale frames in scrollback. Each of the three TUIs prepended a full-screen erase (`ESC[2J` + `ESC[H`) to every rendered frame; on most terminals `ESC[2J` dumps the erased screen into scrollback, so each redraw visually scrolled a page. They now draw on the **alternate screen buffer** (`ESC[?1049h` on enter, `ESC[?1049l` on exit) and reset each frame in place with home + clear-to-end (`ESC[H` + `ESC[0J`) instead — leaving the user's scrollback untouched and restoring the pre-TUI screen cleanly on exit. The three duplicated `CLEAR_SCREEN`/cursor constants are deduped into one shared module, `src/tui/screen.ts` (re-exported from the `src/tui` barrel), so all three TUIs share the same redraw strategy. The select/single-select frame builders are extracted as pure `renderSelectFrame`/`renderSelectOneFrame` functions so the emitted sequences are unit-testable; new tests (`tests/tui/screen.test.ts`, `tests/tui/render-no-scroll.test.ts`) assert no frame emits `ESC[2J`. The scroll behavior itself is only observable in a real terminal, so it was also verified manually. (spry-rp65)
- Cassettes: PR-number normalization (`src/lib/recording-client.ts`) now also recognizes the bare `#<n>` references a spry `spry:stack-links` body section renders (e.g. `1\. #1234 ← this PR`, from `generateStackLinks`), not just `pull/<n>` URLs and `"number":<n>` JSON fields. Recording a PR body containing stack-links left the raw GitHub-minted PR number embedded in a recorded `gh` call's stdin/stdout — a replay match key — so replaying that cassette against a fresh clone (a different real PR number) failed with "No matching recorded entry". Covered by two new unit tests in `tests/lib/recording-client.test.ts` (normalizes the stack-links form; leaves an unrelated bare `#<n>` — no preceding ordinal marker — untouched). (No `sp` runtime change; the recording client is the test-only record seam.)
