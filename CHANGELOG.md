...
- `sp group --apply`'s input schema (`src/parse/apply-doc.ts`) now makes `sp view --json` output valid `--apply` input **verbatim**, so `sp view --json | sp group --apply -` round-trips with no field-stripping. Two changes: (1) the input PR directive field is renamed from `pr` to `prAction` (`"prAction": "CLOSE" | "ADOPT"`), distinct from the output `pr` state object so the two never collide; (2) the three output-only fields `view --json` emits on every node — `sha`, `subject`, and `pr` — are now silently **ignored** on input rather than rejected (previously any `sha` on input was a hard parse error). The internal parsed representation still calls the field `pr` (`ParsedCommit.pr`/`ParsedGroup.pr`); only the JSON key read from input changed.
