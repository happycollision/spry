...
- `spry.autoDeleteOnLand` git-config setting (boolean, default `false`) — read by `readConfig` into `SpryConfig.autoDeleteOnLand`. When true, a follow-up will have `sp land` delete the remote branches of units it just landed; it is opt-in because some repos already have GitHub auto-delete head branches on merge. Parsed via `git config --get --type=bool`, so truthy values (`true`/`1`/`yes`/`on`) become true and unset, empty, falsy, or invalid values all resolve to false without throwing. No runtime behavior change yet — this commit only adds the config plumbing.
